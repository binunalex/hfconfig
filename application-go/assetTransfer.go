...
func main() {
	...
	contract := network.GetContract("sacc")

	log.Println("--> Submit Transaction: InitLedger, function creates the initial set of assets on the ledger")
	result, err := contract.SubmitTransaction("set","a","20")
	if err != nil {
	...
	result, err = contract.SubmitTransaction("set", "a", "1300")
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
	log.Println(string(result))
	...
	result, err = contract.EvaluateTransaction("get", "a")
	if err != nil {
...
func populateWallet(wallet *gateway.Wallet) error {
	...
	certPath := filepath.Join(credPath, "signcerts", "User1@org1.dredev.de-cert.pem")
